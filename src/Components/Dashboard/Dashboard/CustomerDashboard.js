  ...
  useEffect(() => {
    ...
    const getOrders = async () => {
      const res = await fetch(
        `https://frozen-depths-33463.herokuapp.com/order?email=` + user.email
      );
          ...
          <Table responsive className="table-borderless table-hover">
            <thead className="bg-light">
              <tr>
                ...
                <th className="fw-bolder">OrderTime</th>
                <th className="fw-bolder">Status</th>
                <th className="fw-bolder">Method</th>
            ...
            <tbody>
              {orders.map((order, i) => (
                <tr key={i + 1}>
                  <td className="h6 text-muted ">{order._id.substring(20, 24)}</td>
                  <td className="h6 text-muted">{order.orderTime}</td>
                  <td className="h6 text-muted">{order.status}</td>
                  <td className="h6 text-muted">{order.method}</td>
                  <td className="h6 text-muted">${order.total}</td>
                </tr>
              ))}
